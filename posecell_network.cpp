...
{
/**
 * @brief Construct a new Posecell Network:: Posecell Network object. Set the mid point of pose cell array as the starting position.Initialize the current_vt, current_exp, odo_update and vt_update. Build arrays for different functions. 
 * 
 * @param settings (parameters from the config file)
 */
...
/**
 * @brief Build array spaces for mutiple purpose: 
 * posecell[][][]: PC_DIM_XY * PC_DIM_XY * PC_DIM_TH(Posecell/double); 
 * pca_new[][][]: PC_DIM_XY * PC_DIM_XY * PC_DIM_TH(Posecell/double); 
 * pca_new_rot_ptr[][]: (PC_DIM_XY + 2)*(PC_DIM_XY + 2)(Posecell/double); 
 * pca_new_rot_ptr2[][]: (PC_DIM_XY + 2)*(PC_DIM_XY + 2)(Posecell/double); 
 * posecells_plane_th[]: (PC_DIM_XY + 2) * (PC_DIM_XY + 2)(Posecell/double); 
 * 
 * posecells_memory[]: PC_DIM_XY * PC_DIM_XY * PC_DIM_TH(Posecell/double);
 * pca_new_memory[]: PC_DIM_XY * PC_DIM_XY * PC_DIM_TH(Posecell/double);
 * 
 * 3d normalized arrays, with centre at the middle, for exication and inhibition
 * PC_W_EXCITE[][][]: (PC_W_E_DIM * PC_W_E_DIM * PC_W_E_DIM)(double);
 * PC_W_INHIB[][][]: (PC_W_I_DIM * PC_W_I_DIM * PC_W_I_DIM)(double);
 * 
 * Arrays that are used to cast to the wrapped 3d pose cells 
 * PC_E_XY_WRAP[]: (PC_DIM_XY + PC_W_E_DIM - 1)(int));
 * PC_E_TH_WRAP[]: (PC_DIM_TH + PC_W_E_DIM - 1)(int));
 * PC_I_XY_WRAP[]: (PC_DIM_XY + PC_W_I_DIM - 1)(int));
 * PC_I_TH_WRAP[]: (PC_DIM_TH + PC_W_I_DIM - 1)(int));
 * PC_AVG_XY_WRAP[]: (PC_DIM_XY + 2 * PC_CELLS_TO_AVG)(int));
 * PC_AVG_TH_WRAP[]: (PC_DIM_TH + 2 * PC_CELLS_TO_AVG)(int));
 * 
 * Arrays that are used to cast each different position to its sine and cosine value
 * PC_XY_SUM_SIN_LOOKUP[]: PC_DIM_XY(double));
 * PC_XY_SUM_COS_LOOKUP[]: PC_DIM_XY(double));
 * PC_TH_SUM_SIN_LOOKUP[]: PC_DIM_TH(double));
 * PC_TH_SUM_COS_LOOKUP[]: PC_DIM_TH(double));
 */
...
/**
 * @brief Add energy to posecells[act_z][act_y][act_x]
 * 
 */
...
/**
 * @brief for every pose cell, multiply the current energy by a pdf to spread the energy (PC_W_EXCITE[] is a 3d pdf), store the energy in pca_new[][][]
 * 
 * @return true 
 * @return false 
 */
...
/**
 * @brief for every pose cell, multiply the current energy by a pdf to spread the energy (PC_W_INHIB[] is a 3d pdf), store the energy in pca_new[][][]
 * 
 * @return true 
 * @return false 
 */
...
/**
 * @brief Reduce all posecell_memory by PC_GLOBAL_INHIB(0.00002), keep minmum value at 0
 * 
 * @return true 
 * @return false 
 */
...
/**
 * @brief divide each posecell_element by their total value
 * 
 * @return true 
 * @return false 
 */
...
/**
 * @brief Do path integration according to the vtran and vrot input, haven't completely understand it yet
 * 
 * @return true 
 * @return false 
 */
bool PosecellNetwork::path_integration(double vtrans, double vrot)
{
  ...
  for (dir_pc = 0; dir_pc < PC_DIM_TH; dir_pc++)
  {
    ...
    // PC_C_SIZE_TH = (2.0 * M_PI) / PC_DIM_TH;   rad/unit
    double dir = dir_pc * PC_C_SIZE_TH + angle_to_add;  // rad
    ...
    // pca_new_memory[0:(PC_DIM_XY + 2) * (PC_DIM_XY + 2)] = zeros()
    memset(pca_new_memory, 0, sizeof(double) * (PC_DIM_XY + 2) * (PC_DIM_XY + 2));

    // pca_new(2:end-1,2:end-1) = pca90;
    // copy posecell[dir_pc][0:PC_DIM_XY][0] to pca_new_rot_ptr[1:PC_DIM_XY+1][1]
    for (j = 0; j < PC_DIM_XY; j++)
    {
...
/**
 * @brief find the best (x, y, theta), haven't completely understand it yet
 * 
 * @return double 
 */
double PosecellNetwork::find_best()
{
  ...
  // find the max activated cell in pose cells and return x, y, theta
  double max = 0;
  for (k = 0; k < PC_DIM_TH; k++)
  {
    for (j = 0; j < PC_DIM_XY; j++)
...
/**
 * @brief Calculate the Euclidean distance between best pose cell position and current position
 * 
 * @return double 
 */
  ...
  if (rot % 2 == 1)
  {
    for (j = 0; j < (int)(centre) + (1 - dim % 2); j++)
    {
      for (i = 0; i < (int)centre + 1; i++)
      {
        id = i;
        jd = j;
        // set tmp_old equal to posecell[N][j][i]
        tmp_old = array[j][i];
...
/**
 * @brief Normalization the 3d coordinate (x, y, z) with regrad to dim_centre
 * 
 * @return double 
 */
...
/**
 * @brief Enlarge the experience array size by 1. Set the coordinate of new experience node to the most possible position in THE pose cell. Set vt_id equal to current view template id. Store id of current experience map in the related experience array of the view template.
 * 
 */
...
/**
 * @brief Get the next action for experience map. Start from current view template, compare all associated experiences (except current experience) with the best pose cell position. When the distance is short enough set the matched experience as node, otherwise, create a new node for current node.
 * 
 * @return PosecellNetwork::PosecellAction 
 */
PosecellNetwork::PosecellAction PosecellNetwork::get_action()
{
  ...
  {
    // create a new node is no experience exists
    create_experience();
    action = CREATE_NODE;
  } else {
    ...
    // compare the distance between current experience node and pose cell position
    delta_pc = get_delta_pc(experience->x_pc, experience->y_pc, experience->th_pc);

    PosecellVisualTemplate * pcvt = &visual_templates[current_vt];
    if (pcvt->exps.size() == 0)
    {
      // if current view template doesn't have any experience map, create a new one
      create_experience();
      action = CREATE_NODE;
     }
    else if (delta_pc > EXP_DELTA_PC_THRESHOLD || current_vt != prev_vt)
    {
      // if distance is large enough or view has changed, go through all the exps associated with the current view and find the one with the closest delta_pc
      ...
      // Search through all experiences associated with current view and find the closest experience to current pose cell position
      ...
      // if an experience is closer than the threshold, set it as matched experience and create a link
      ...
      if (current_exp != (unsigned)matched_exp_id)
      {
        if (matched_exp_id == -1)
        {
          // if no experience is matched, create a new node
          create_experience();
          action = CREATE_NODE;
        }
        else
        {
          // if there is one matched experience, set that experience to current experience, set that node as current node 
...
/**
 * @brief Excite, inhibit and normalize the pose cells, then conduct the path integration based on vtran, vrot and delta time to shift energy among pose cells and find the best pose cell(x, y, th)
 * 
 * @param vtrans linear velocity
 * @param vrot angular velocity
 * @param time_diff_s time past since last calculation
 */
void PosecellNetwork::on_odo(double vtrans, double vrot, double time_diff_s)
{
  // linear movement, rotation in time_diff_s
...
/**
 * @brief set image cooridnate as the most possible pose cell position
 * 
 */
...
/**
 * @brief Create a new view template when new view appearing. Inject energy to current pose cell position
 * 
 * @param vt 
 * @param vt_rad 
 */
void PosecellNetwork::on_view_template(unsigned int vt, double vt_rad)
{
  ...
  {
    // when a new view template comes in, create a new view template
    create_view_template();
    assert(vt == visual_templates.size()-1);
  }
  else
  {
    // if the coming view template is a familiar one, find it from the Posecell Visual Template vector
    ...
    if (vt < (visual_templates.size() - 10))
    {
      if (vt != current_vt)
      {
        ...
        // add the decay if the view doesn't change
        pcvt->decay += VT_ACTIVE_DECAY;
      }
