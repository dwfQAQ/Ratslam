...
{
/**
 * @brief store id(uint), pixel data[], mean value(double) of a single image
 * 
 */
...
/**
 * @brief Get the current_vt
 * 
 * @return current_vt(int)
 */
  int get_current_vt()
  {
    return current_vt;
...
private:
  ...
  /**
   * @brief Set the current_vt to the id of the matched template, or create new template and set current_vt to new id
   * 
   * @param id (matched template id)
   */
  ...
  // the vector that is used to save all view templates
  std::vector<VisualTemplate> templates;

  // the vector to save the processed image
