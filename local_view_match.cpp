...
/**
 * @brief Construct a new Local View Match:: Local View Match object. reserve 10000 templates, and a image space for current view. Initialize current_vt and prev_vt
 * 
...
/**
 * @brief Main part of local view template function. Input image preprocessing. Compare the image with all view templates. Match the image with existed templates or make it a new view template. Set current_vt to the best matched view template of the image.
 * 
 * @param view_rgb image pixel array
 * @param greyscale 
 * @param image_width 
 * @param image_height 
 */
void LocalViewMatch::on_image(const unsigned char *view_rgb, bool greyscale, unsigned int image_width, unsigned int image_height)
{
  ...
  // set last matched view template id as the previous one, leave current_vt for the new match
  prev_vt = get_current_vt();
  unsigned int vt_match_id;
  compare(vt_error, vt_match_id);

  // if the error between matched template and current view is larger than the VT_MATCH_THRESHOLD, make the image a new template. If it is smaller, set this image as a new template.
  // current_vt should always be equal to the best matched view template of current image
...
/**
 * @brief Preprocess the new image. Downsample an image and convert it to grayscale(0 ~ 1.0);global normalization and patch normalization;calculate the mean of the whole image in the end; original image is stored in view_rgb[], processed image is saved in current_view[]
...
/**
 * @brief create and add a visual template to the collection. Add one size to the templates[]. The id of new template is size-1, data is the processed image, and the mean.
 * 
...
/**
 * @brief Search through all view templates. Compare the difference between current view(trimmed the shift match parts) and templates(with offset) pixel by pixel. Sum of all pixel difference it cdiff. The minmum difference is stored in mindiff. By going through all templates, the best matching template is returned as vt_match_id and the vt_err.
 * 
 * @param vt_err the average error of each pixel in current image by comparing it difference with its most similar template
 * @param vt_match_id the template that best match to current view
 */
void LocalViewMatch::compare(double &vt_err, unsigned int &vt_match_id)
{
  ...
  // save the processed current image into data[]
  ...
  if (VT_PANORAMIC)
  {
	...
	for (offset = 0; offset < VT_SHIFT_MATCH*2+1; offset += VT_STEP_MATCH)
	{
      ...
      for (column_ptr = column_row_ptr, template_ptr = template_row_ptr; column_ptr < column_row_ptr + sub_row_size; column_ptr++, template_ptr++)
      {
        // sum up difference between templates and current image
        cdiff += abs(*column_ptr - *template_ptr);
      }

      // fast breaks based on error already great than previous errors
      if (cdiff > mindiff)
        break;
	  }

    // make current image as the best match, record its id as min_id (temporarily)
	  if (cdiff < mindiff)
	  {
      mindiff = cdiff;
      min_template = vt.id;
	...
	vt_relative_rad = 0;
  // average the total difference to each pixel
