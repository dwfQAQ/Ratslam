...
void odo_callback(nav_msgs::OdometryConstPtr odo, ratslam::PosecellNetwork *pc, ros::Publisher * pub_pc)
{
  ...
  if (prev_time.toSec() > 0)
  {
    ...
    // set the source experience id before processing new movement
    pc_output.src_id = pc->get_current_exp_id();

    // modify the energy of each cell in the pose cell according to velocity
    pc->on_odo(odo->twist.twist.linear.x, odo->twist.twist.angular.z, time_diff);

    // get action for the building of experience map
    pc_output.action = pc->get_action();

    // set the TopologicalAction message, header, destination experience id, relative rad and publish it
